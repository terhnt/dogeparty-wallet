function MessageFeed() {
  var self = this;
  self.lastMessageIndexReceived = ko.observable(0); //message index = the message feed index generated by unoparty-server (mempool tx do not have indexes and indexes CAN shift back during reorgs)
  self.lastMessageSeqReceived = ko.observable(0); //message seq = the message feed sequence number generated by unoblock's unowallet module (mempool tx DO have indexes, and seqs DO NOT shift back during reorgs)
  ...
  self.parseMessage = function(seq, when, message) {
    ...
    //NOTE: we use the message sequence that the unoblock unowallet module generates, as it takes into account mempool transactions
    // (which come across the unoblock unowallet feed like any other message)
    // the message_index created by the unoparty-server message feed DOES NOT generate message_indexes for mempool transactions
    ...
    if (message['_command'] == 'reorg') {
      ...
      setTimeout(function() { WALLET.refreshUnopartyBalances(WALLET.getAddressesList(), checkURL); }, randomIntFromInterval(1, 5) * 1000);
