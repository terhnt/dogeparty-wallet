...
/***********
 * unowallet.conf.json LOADING AND SERVICES INITIALIZATION
 ***********/
...
function initRollbar() {
  /* TODO: Try to load rollbar earlier, possibly... (However, as we get the accessToken from unowallet.conf.json, we'd have
...
function loadUnowalletConfigFromServer() {
  ...
  $.getJSON(UNOWALLET_CONF_LOCATION, function(data) {
    ...
    assert(data['servers'] && data['servers'] instanceof Array, "'servers' field in returned unowallet.conf.json file is not an array");
    ...
    if (data['disabledFeatures']) {
      assert(data['disabledFeatures'] instanceof Array, "'disabledFeatures' field in returned unowallet.conf.json file is not an array");
    ...
    if (data['restrictedAreas']) {
      assert(data['restrictedAreas'] instanceof Object, "'restrictedAreas' field in returned unowallet.conf.json file is not an object");
