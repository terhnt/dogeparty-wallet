...
function getAssetImageURL(asset) {
  ...
  if(official) {
    ...
    $.getJSON(projectJSON, function(data) {
      obj = JSON.parse(data);
      imageurl = obj[asset].image_url;
    });
...
function isOfficialProject(asset) {
  ...
  var obj;
  $.getJSON('https://terhnt.github.io/unoprojectfeed.json', function(data) {
    obj = JSON.parse(data);
  ...
  while(i < jsonurls.length) {
    var x = 0; // Asset #
    $.getJSON(obj.Project.Url[i], function(pdata) {
      const projobj = JSON.parse(pdata);
      var len = Object.keys(projobj);
      while(x < len){
        if(asset.toUpperCase() === Object.keys(projobj)[x]){
          return true;
        }
        x++;
      }
...
function getOfficialProject(asset) {
  ...
  while(i < jsonurls.length) {
    var x = 0; // Asset #
    $.getJSON(obj.Project.Url[i], function(pdata) {
      var projobj = JSON.parse(pdata);
      var len = Object.keys(projobj);
      while(x < len){
        if(asset.toUpperCase() === Object.keys(projobj)[x]){
          return obj.Projects.Names[i];
        }
        x++;
      }
    });
...
function getOfficialProjectUrlJSON(projectname) {
  ...
  $.getJSON('https://terhnt.github.io/unoprojectfeed.json', function(data) {
    ...
    while(i < pros.length){
      if(projectname === pros[i]){
        return obj.Projects.Url[i];
      }
      i++;
    }
  });
