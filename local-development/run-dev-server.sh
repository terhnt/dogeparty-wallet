...
# runs a local nginx server that proxies requests to a remote unowallet instance
...
export DEV_REMOTE_HOST=${DEV_REMOTE_HOST:=wallet.unowallet.io}
...
envsubst "$VARS" < ./etc/includes/unowallet.conf.template > ./etc/includes/unowallet.conf
